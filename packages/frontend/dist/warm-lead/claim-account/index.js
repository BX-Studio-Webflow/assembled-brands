...
async function apiHotLeadRegister(data) {
...
// warm-lead/claim-account/index.ts
var initClaimAccountPage = () => {
  processMiddleware();
  ...
  const workEmail = queryElement('[dev-target="work-email-input"]', form);
  const firstName = queryElement('[dev-target="first-name-input"]', form);
  const lastName = queryElement('[dev-target="last-name-input"]', form);
  const password = queryElement('[dev-target="password-input"]', form);
  const loanUrgencyRadios = queryAllElements('input[name="loan_urgency"]', form);
  ...
  if (!loanUrgencyRadios.length) {
    console.error(
      'Ensure [input[name="loan_urgency"]] is present. Found ' + loanUrgencyRadios.length + " elements."
    );
    return;
  }
  if (!submitButton) {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    const payload = {
      ...
      loan_urgency: selectedLoanUrgency.value
    };
    try {
