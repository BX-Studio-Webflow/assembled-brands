...
// shared/services/DealApplicationService.ts
async function apiGetMyDealApplications() {
  return ApiService_default.fetchDataWithAxios({
    url: "/deal-applications",
    method: "get"
  });
}
async function apiExchangeWarmLeadSession(dealId) {
  return ApiService_default.fetchDataWithAxios({
...
var FINANCIAL_WIZARD_PAGES_FOR_WARM = [
  "accounts-inventory",
  "ecommerce-performance",
  "team-ownership"
];
var isWarmWizardDocumentStepComplete = (page, progress) => {
...
var isWarmFinancialReportStepComplete = (progress) => {
  const reports = progress?.financial_reports ?? [];
  return reports.some((doc) => doc.document_type === "monthly_balance_sheet") && reports.some((doc) => doc.document_type === "monthly_income_statement");
};
var isWarmForecastStepComplete = (progress) => {
  const reports = progress?.financial_reports ?? [];
  return reports.some((doc) => doc.document_type === "income_statement_forecast") && reports.some((doc) => doc.document_type === "balance_sheet_full_year_forecast");
};
var resolveWarmDealApplicationPath = (dealId, onboardingApplication, financialProgress) => {
...
// warm-lead/finance-my-applications/index.ts
var formatDealApplicationStatus = (status) => {
  switch (status) {
    case "active":
      return "Active";
    case "submitted":
...
var populateApplicationRow = (row, application) => {
  const dealNameCell = queryElement('[dev-target="deal-name"]', row);
  const statusCell = queryElement('[dev-target="status"]', row);
  const createdAtCell = queryElement('[dev-target="created-at"]', row);
  const updatedAtCell = queryElement('[dev-target="updated-at"]', row);
  if (dealNameCell) {
...
var openApplication = async (dealId) => {
...
var initFinanceMyApplicationsPage = async () => {
  ...
  const newApplicationLink = queryElement(
