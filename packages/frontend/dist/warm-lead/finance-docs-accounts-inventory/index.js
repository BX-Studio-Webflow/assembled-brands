...
var buildBusinessUpdatePayload = (profile, inventory) => ({
  ...
  international_location: inventory.inventory_location === "International" ? inventory.international_location ?? "" : ""
});
var initFinanceDocsAccountsInventoryPage = async () => {
  ...
  const inventoryLocationSelect = queryElement(
    'select[dev-target="company-hq"]',
    form
  );
  const internationalLocationInput = queryElement(
  ...
  const updateInventoryLocationFields = (progress) => {
  ...
  const getInventoryLocationPayload = () => {
    ...
    return {
      inventory_location: inventoryLocationSelect.value,
      international_location: isInternationalInventoryLocation() ? internationalLocationInput.value.trim() : void 0
    };
  };
  const saveInventoryLocation = async () => {
    ...
    if (!inventoryPayload) {
      throw new Error(
        isInternationalInventoryLocation() ? "Please specify the city and country" : "Please select an inventory location"
      );
    }
  ...
  form.addEventListener("submit", async (event) => {
    ...
    if (!inventoryPayload) {
      submitButton.classList.add("is-error");
      submitButton.value = isInternationalInventoryLocation() ? "Please specify the city and country" : "Please select an inventory location";
      return;
    }
