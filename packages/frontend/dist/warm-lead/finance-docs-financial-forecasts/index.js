...
// warm-lead/finance-docs-financial-forecasts/index.ts
var ALLOWED_FILE_TYPES = [...WARM_LEAD_EXCEL_MIME_TYPES];
var initFinanceForecastsPage = async () => {
  ...
  const balanceForecastBox = queryElement(
    '[dev-target="balance-forecast-upload-box"]',
    form
  );
  const balanceForecastInput = queryElement(
    '[dev-target="file-input"]',
    balanceForecastBox ?? form
  );
  const balanceForecastHelpText = queryElement(
  ...
  if (missingElements || !incomeForecastBox || !incomeForecastInput || !incomeForecastHelpText || !balanceForecastBox || !balanceForecastInput || !balanceForecastHelpText || !submitButton) {
    return;
  }
  configureWarmLeadExcelFileInputs(incomeForecastInput, balanceForecastInput);
  ...
  const getFinancialReportDoc = (documentType) => financialProgress?.financial_reports?.find((doc) => doc.document_type === documentType);
  ...
  incomeForecastInput.addEventListener(
    "change",
    () => updateHelperText(incomeForecastInput, incomeForecastHelpText)
  );
  ...
  balanceForecastBox.addEventListener(
    "dragleave",
  ...
  balanceForecastInput.addEventListener(
    "change",
    () => updateHelperText(balanceForecastInput, balanceForecastHelpText)
  );
