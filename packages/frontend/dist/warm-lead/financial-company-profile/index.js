...
// warm-lead/financial-company-profile/index.ts
var initFinancialCompanyProfilePage = async () => {
  ...
  const companyLegalNameInput = queryElement(
    '[dev-target="company-legal-name"]',
    form
  );
  const companyHeadquartersInput = queryElement(
    '[dev-target="company-hq"]',
    form
  );
  const companyYear = queryElement('[dev-target="company-year"]', form);
  const accountingSoftwareInput = queryElement(
    '[dev-target="accounting-software"]',
    form
  );
  const accountingSoftwareOther = queryElement(
    '[dev-target="other-accounting-software"]',
    form
  );
  const accountingSoftwareOtherWrapper = queryElement(
  ...
  const updateFormFields = (progress, onboardingProgress2) => {
    const legalNameFromCompanyProfile = progress?.company_profile?.legal_name;
    const legalNameFromOnboarding = onboardingProgress2?.progress?.step1?.legal_name;
    const legalNameFromBusiness = progress?.business?.legal_name;
    const resolvedLegalName = legalNameFromCompanyProfile || legalNameFromOnboarding || legalNameFromBusiness;
