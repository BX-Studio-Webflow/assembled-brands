...
var initWarmLeadOnboardingPage = async () => {
  ...
  const hasValidUrlDealId = Number.isInteger(urlDealId) && urlDealId > 0;
  const dealId = hasValidUrlDealId ? urlDealId : null;
  ...
  const companyHq = queryElement('select[dev-target="company-hq"]', form);
  const netRevenue = queryElement('[dev-target="company-net-revenue"]', form);
  const memberRadios = Array.from(
    form.querySelectorAll('input[name="working-with-member"]')
  );
  const memberSelect = queryElement('select[dev-target="which-member"]', form);
  const memberFieldWrapper = memberSelect?.closest(".input-onboarding_wrapper");
  if (!submitButton || !backButton || !legalName || !companyHq || !netRevenue || memberRadios.length === 0 || !memberSelect) {
  ...
  const handleSubmit = async () => {
    ...
    const workingWith = form.querySelector(
      'input[name="working-with-member"]:checked'
    );
