...
var initInviteTeamMembersPage = async () => {
  ...
  const nameInput = queryElement('[dev-target="name"]', form);
  const emailInput = queryElement('[dev-target="email"]', form);
  const roleInput = queryElement('[dev-target="role"]', form);
  const inviteMessageInput = queryElement('[dev-target="invite-message"]', form);
  const addAnotherMemberLink = queryElement(
    '[dev-target="add-another-member"]',
    form
  );
  const submitButton = queryElement('[dev-target="back-button"]', form);
  if (!nameInput) {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    memberGroups.forEach((group) => {
      if (group instanceof HTMLElement) {
        const groupEmailInput = queryElement('[dev-target="email"]', group);
