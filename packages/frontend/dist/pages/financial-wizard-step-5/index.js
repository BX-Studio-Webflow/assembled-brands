...
var apiCompleteFinancialApplication = () => {
  return ApiService_default.fetchDataWithAxios({
...
// pages/financial-wizard-step-5/index.ts
var initFinancialWizardStep5Page = () => {
  processMiddleware();
  const form = document.querySelector('[dev-target="financial-wizard-step5-form"]');
  if (!form) {
    console.error(
      'Financial Wizard Step 5 form not found. Element: [dev-target="financial-wizard-step5-form"] not found'
    );
    return;
  }
  ...
  const completeButton = queryElement('[dev-target="complete-button"]', form);
  ...
  if (!completeButton) {
    console.error('Ensure [dev-target="complete-button"] is present.');
    return;
  }
  ...
  completeButton.addEventListener("click", async () => {
    try {
      await apiCompleteFinancialApplication();
      completeButton.classList.add("is-success");
      completeButton.value = "Application completed!";
      setTimeout(() => {
      ...
      completeButton.classList.add("is-error");
      completeButton.value = message || "There was a problem completing the application";
    }
  });
