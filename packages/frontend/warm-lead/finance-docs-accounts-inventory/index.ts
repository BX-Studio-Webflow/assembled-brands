...
const initFinanceDocsAccountsInventoryPage = async () => {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    const hasMonthlyInventoryDoc =
      Boolean(monthlyInventoryInput.files?.[0]) || Boolean(getDoc('monthly_inventory_report'));
    const hasAccountsReceivableDoc =
      Boolean(accountsReceivableInput.files?.[0]) || Boolean(getDoc('accounts_receivable_aging'));
    const hasAccountsPayableDoc =
      Boolean(accountsPayableInput.files?.[0]) || Boolean(getDoc('accounts_payable_aging'));
