...
const initFinanceForecastsPage = async () => {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    const hasIncomeForecastDoc =
      Boolean(incomeForecastInput.files?.[0]) ||
      Boolean(getFinancialReportDoc('income_statement_forecast'));
    const hasBalanceForecastDoc =
      Boolean(balanceForecastInput.files?.[0]) ||
      Boolean(getFinancialReportDoc('balance_sheet_full_year_forecast'));
