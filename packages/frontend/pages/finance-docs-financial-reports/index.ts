...
const initFinanceReportsPage = async () => {
    ...
    if (progress?.financial_reports) {
      ...
      if (incomeStatement) {
        incomeStatementHelpText.textContent =
          incomeStatement.asset_name || 'Supported formats: sheets. xcel';
      } else {
      ...
      if (incomeForecast) {
        incomeForecastHelpText.textContent =
          incomeForecast.asset_name || 'Supported formats: sheets. xcel';
      } else {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    const hasIncomeForecastDoc =
      ...
      Boolean(getFinancialReportDoc('monthly_income_forecast'));
