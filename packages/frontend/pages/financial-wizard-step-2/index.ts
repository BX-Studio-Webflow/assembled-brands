...
const initFinancialWizardStep2Page = () => {
  processMiddleware();

  const form = document.querySelector('[dev-target="financial-wizard-step2-form"]');
  if (!form) {
    console.error(
      'Financial Wizard Step 2 form not found. Element: [dev-target="financial-wizard-step2-form"] not found'
    );
    return;
  }
  ...
  backButton.addEventListener('click', () => {
    navigateToPath('/financial-wizard-step-1');
  });
