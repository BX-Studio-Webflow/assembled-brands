...
const initFinanceDocsAccountsInventoryPage = async () => {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    const hasMonthlyInventoryDoc =
      Boolean(monthlyInventoryInput?.files?.[0]) ||
      Boolean(getAccountsInventoryDoc('monthly_inventory_report'));
    const hasAccountsReceivableDoc =
      Boolean(accountsReceivableInput?.files?.[0]) ||
      Boolean(getAccountsInventoryDoc('accounts_receivable_aging'));
    const hasAccountsPayableDoc =
      Boolean(accountsPayableInput?.files?.[0]) ||
      Boolean(getAccountsInventoryDoc('accounts_payable_aging'));
