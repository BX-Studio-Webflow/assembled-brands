...
export default async function EcommercePerformancePage() {
  const cookieStore = await cookies();
  const token = cookieStore.get(ACCESS_TOKEN_COOKIE)?.value ?? null;
  if (!token) redirect("/onboarding");

  const data = await loadDocumentPageData("ecommerce-performance", token);

  return (
    <AppShell
      activeKey="ecommerce-performance"
